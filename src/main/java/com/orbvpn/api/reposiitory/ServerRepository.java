...
public interface ServerRepository extends JpaRepository<Server, Integer> {
    ...
    @Query(value = "SELECT server.* FROM radacct JOIN server ON radacct.nasipaddress = server.private_ip where radacct.username=?1 GROUP BY nasipaddress order by acctupdatetime DESC", nativeQuery=true)
